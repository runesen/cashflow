...
def _plot_stacked_bars(
    ...
    for c in components:
        c_amounts = sign * c.summary[(c.summary.index >= from_date) & (c.summary.index <= to_date)]['amount']
        c_value = c_amounts.sum() if agg == "sum" else c_amounts.mean() if agg == "mean" else logger.error("Unknown aggregation")
        ax.bar(x=c.plot_position, height=c_value, width=.75, label=c.name, bottom=components_sum, color=c.color)
...
def _plot_stacked_curves(
    ...
    for o in objects:
        ...
        ax.fill_between(x=sum_of_values.index, y1=sum_of_values, y2=sum_of_values_new, color=o.color)
